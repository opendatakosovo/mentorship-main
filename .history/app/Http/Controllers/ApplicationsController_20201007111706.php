...
class ApplicationsController extends Controller
{
 ...
 public function update_client_file($id)
 {
  $user = Client::find($id);
  ...
  return view('entry')->with('data', $data);
 }

 public function admin_update_cv(Request $request, $id)
 {
//   $id = $request->client_id;
  $user = Client::find($id);

  $filename = $request->file('cv')->store('cv/' . $request->email);
  if ($request->hasFile('application/pdf')) {foreach ($request->file('files') as $cv) {
   // $path = $request->file('avatar')->storeAs(
   //     'avatars', $request->user()->id
   // );
   $file_names[] = $cv->originalName;

   Client::create([
    'mentor_id' => $request->client_id,
    'filename' => $filename,
    'client' => $request,
   ]);
  }
   $mentor_name = get_mentor_name($request->client_id);
  }
// dd($filename);
